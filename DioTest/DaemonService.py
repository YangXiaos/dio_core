...
from DioCore.Utils import SendEmailUtil
...
class DaemonService(object):
    ...
    def test_location_service(self, request_url):
        try:
            ...
            if self.counter[request_url] >= 3:
                SendEmailUtil.sendEmail("178069857@qq.com", "服务 {} 运行超过3次异常".format(request_url), subject="[淘宝Xsign服务异常] 线上服务异常")
    ...
    def test_online_service(self, request_url):
        try:
            ...
            self.counter[request_url] += 1
            if self.counter[request_url] >= 3:
                SendEmailUtil.sendEmail("178069857@qq.com", "服务 {} 运行超过3次异常".format(request_url), subject="[淘宝Xsign服务异常] 本地服务异常")
            print("{} 接口服务异常".format(request_url))
