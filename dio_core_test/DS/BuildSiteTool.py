...
from dio_core.utils import url_util, json_util
...
def buildNavTemplate(parentId, navUrls: list, regex="", match_regex= ""):
    ...
    tpId = buildTemplate({"parentId": parentId, "template": json_util.to_json(template)})
...
def buildContentTemplate(parentId, navUrls: list, regex=""):
    ...
    rules = [
        {"id": "7", "param": ""},
        {
            "id": "11",
            "name": "通用新闻正文抽取处理器",
            "param": json_util.to_json({"url_key": "full_url", "useLegacy": "false"})
        },
        {
            "id": "4",
            "name": "内容抽取处理器",
