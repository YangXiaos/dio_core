...
def main(time: str, requestNum: int, tp: str="", ):
    h = text_util.get_first_match(time, "(\d+)h,")
    m = text_util.get_first_match(time, "(\d+)m")
    s = text_util.get_first_match(time, "(\d+)s")
...
def main2(time: str, time2: str, requestNum: int, requestNum2: int, tp: str="", ):
    ...
    h2 = text_util.get_first_match(time2, "(\d+)h,")
    m2 = text_util.get_first_match(time2, "(\d+)m")
    s2 = text_util.get_first_match(time2, "(\d+)s")
