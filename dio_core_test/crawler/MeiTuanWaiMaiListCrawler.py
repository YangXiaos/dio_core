...
# ttfUrl = "http://"+ TextUtil.get_first_match(res.text, "format\(\"embedded-opentype\"\),url\(\"//(.*?\.woff)\"\)")
...
def crawlShopList():
    ...
    while True:
        # download 解析
        res = Downloader.post(SHOP_SEARCH_URL.format(time_util.get_unix()), post_data_of_shop_list, setting=setting)
        jsonObject = res.json()
...
def crawlComment(shopId: str):
    ...
    try:
        for comment in jsonObject["data"]["list"]:
            ...
            md5 = md5_util.md5("_".join([comment["commentTime"], comment["content"], str(comment["score"]), comment["deliveryTime"]]))
